...
app = FastAPI(title="reconciliation_service")
...
@app.post("/process")
def process(payload: ReconciliationServiceRequest, db: Session = Depends(get_db)) -> ServiceEvent:
    record = ProcessingRecord(external_id=payload.external_id, amount_cents=payload.amount_cents)
    db.add(record)
    db.commit()
    event = ServiceEvent(
        event_type="reconciliation_service.received",
        source_service="reconciliation_service",
        payload={"external_id": payload.external_id, "amount_cents": payload.amount_cents},
    )
    publisher.send(queue_name="reconciliation_service", message=event.model_dump_json())
    return event

...
app = FastAPI(title="batch_builder_service")
...
@app.post("/process")
def process(payload: BatchBuilderServiceRequest, db: Session = Depends(get_db)) -> ServiceEvent:
    record = ProcessingRecord(external_id=payload.external_id, amount_cents=payload.amount_cents)
    db.add(record)
    db.commit()
    event = ServiceEvent(
        event_type="batch_builder_service.received",
        source_service="batch_builder_service",
        payload={"external_id": payload.external_id, "amount_cents": payload.amount_cents},
    )
    publisher.send(queue_name="batch_builder_service", message=event.model_dump_json())
    return event
